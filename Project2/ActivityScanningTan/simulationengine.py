...
class Event:
    def __init__(self, timestamp, data):
        ...
        self.futureEventList = PQ.PriorQ() #this is the FEL for the C-type events
        self.finished = False # to see when simulation has finished or not
...
A = 12.0
filename = "{}OutputData.csv".format(int(A))

Cutoff = 10000 #number of cars to simulate
...
def eventHandler(self, event):
        if event[2] == "redLight":
            self.arrival(event[3])
        elif event[2] == "greenLight":
            self.cross(event[3])
...
def arrival(event, q): #to some intersection
    ...
    wait =  0
    if ((now % signal_cycle) > (g+y)): #arrived when red light, triggering car to be put on C-type FEL
        ...
        #for first intersection, street #10
    if curr_inter == 10:
        ...
        if ArrivalCount < Cutoff:
            ...
            new_event = Event(ts, data) #schedule a new B type event
    ...
    new_event_c = Event(ts,data)
    new_event_c.scheduler(self, self.timestamp, "greenLight", self.data)
...
#run for C events, keeps going till its finished 
def run(self):
